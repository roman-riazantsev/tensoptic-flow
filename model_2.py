...
from tensorflow.keras.layers import concatenate, Conv2DTranspose, Conv2D
...
class Model2(tf.keras.Model):
    def __init__(self, name='PWC-net'):
        super(Model2, self).__init__(name=name)
    ...
    def computation_step(self, first_frame_features, second_frame_features, old_flow_features, old_flow,
        ...
        features = Conv2DTranspose(filters=4, kernel_size=4, strides=2, padding='same')(new_flow_features)
        features = tf.nn.leaky_relu(features)
        features = Conv2D(filters=2, kernel_size=3, padding='same')(features)

        flow = Conv2DTranspose(filters=4, kernel_size=4, strides=2, padding='same')(new_flow)
        flow = tf.nn.leaky_relu(flow)
        flow = Conv2D(filters=2, kernel_size=3, padding='same')(flow)
