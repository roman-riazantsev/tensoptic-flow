...
from tensorflow.keras.layers import concatenate, Conv2DTranspose, UpSampling2D, Conv2D
...
class Model4(tf.keras.Model):
    def __init__(self, name='PWC-net'):
        super(Model4, self).__init__(name=name)
    ...
    def computation_step(self, first_frame_features, second_frame_features, old_flow_features, old_flow,
        ...
        features = UpSampling2D()(new_flow_features)
        features = Conv2D(filters=2, kernel_size=3, padding='same')(features)
        flow = UpSampling2D()(new_flow)
